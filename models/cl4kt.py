...
from cluster_friendly_module import ClusterFriendlyModule
...
class CL4KT(Module):
    def __init__(self, num_skills, num_questions, seq_len, **kwargs):
        ...
        self.use_joint_training_module = self.args.get("use_joint_training_module", False)
        ...
        if self.use_joint_training_module:
            self.cluster_friendly_module = ClusterFriendlyModule(
                input_dim=self.hidden_size,
                num_clusters=self.args.get("joint_cluster_k", 3),
                project_dim=self.args.get("joint_cluster_project_dim", self.hidden_size),
                enabled_losses=self.args.get(
                    "joint_training_losses", "soft,kmeans,separation,temporal"
                ),
                loss_weight=self.args.get("joint_cluster_loss_weight", 0.05),
                soft_weight=self.args.get("joint_soft_loss_weight", 1.0),
                kmeans_weight=self.args.get("joint_kmeans_loss_weight", 0.1),
                separation_weight=self.args.get("joint_separation_loss_weight", 0.01),
                temporal_weight=self.args.get("joint_temporal_loss_weight", 0.01),
                student_t_alpha=self.args.get("joint_student_t_alpha", 1.0),
                normalize=self.args.get("joint_normalize", True),
            )
    ...
    def loss(self, feed_dict, out_dict):
        ...
        if self.use_joint_training_module and "sequence_features" in out_dict:
            cluster_out = self.cluster_friendly_module(
                out_dict["sequence_features"],
                out_dict["attention_mask"],
            )
            loss = loss + cluster_out["loss"]
            out_dict["joint_cluster_loss"] = cluster_out["loss"].detach()
            out_dict["joint_cluster_loss_raw"] = cluster_out["total_loss"]

        return loss, len(pred[mask]), true[mask].sum().item()

    def extract_features(self, batch, pool=True):
        """Return CL4KT latent states before the prediction MLP.

        The returned sequence state `x` is produced by the question encoder and
        knowledge retriever, immediately before it is concatenated with the raw
        question embedding and passed to `self.out`. This keeps the original
        hidden representation for downstream clustering instead of using the
        ...
        if not pool:
            return {"sequence_features": x, "attention_mask": attention_mask, "attn": attn}

        mask = attention_mask.unsqueeze(-1).float()
        denom = mask.sum(1).clamp_min(1.0)
        features = (x * mask).sum(1) / denom
        return {"features": features, "sequence_features": x, "attention_mask": attention_mask, "attn": attn}
